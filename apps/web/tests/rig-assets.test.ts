...
describe('trex-sock-r0 socket-era pack (D-024)', () => {
  const dir = join(publicDir, TREX_SOCK_RIG_DEF.packPath);
  const manifest = (): unknown => JSON.parse(readFileSync(join(dir, 'manifest.json'), 'utf8'));

  it('manifest.json validates as a nine-piece parts rig', () => {
    expect(validatePartsManifest(manifest())).toMatchObject({ ok: true });
    expect(PARTS_LAYER_IDS).toHaveLength(9);
  });

  it("the def's layer bounds match the manifest exactly (mesh rest + pattern UVs depend on it)", () => {
    const result = validatePartsManifest(manifest());
    if (!result.ok) throw new Error('manifest invalid');
    for (const layer of result.manifest.layers) {
      expect(TREX_SOCK_RIG_DEF.layerBounds[layer.id as PartsLayerId], `bounds for ${layer.id}`).toEqual(layer.bounds);
    }
  });

  it("the def's pack path and seed match the pack", () => {
    const parsed = manifest() as { patterns: { seed: number } };
    expect(TREX_SOCK_RIG_DEF.packPath).toBe('rigs/trex-sock-r0');
    expect(TREX_SOCK_RIG_DEF.seed).toBe(parsed.patterns.seed);
  });
  ...
  it('layers are grayscale value art shipped directly (no value/ dir in a Template S pack)', () => {
    expect(existsSync(join(dir, 'value'))).toBe(false);
  });
});
