...
> **Active path (2026-07-23): Template S** in `PROMPT-TEMPLATES.md` — the
> socket-era contract (grayscale value parts on socket templates; D-024,
> `../IR-LESSONS.md`). The socket templates live here as
> `socket-template-a-v1.png` / `socket-template-b-{biped,quad}-v1.png`
> (generated by `tools/socket-template/make-template.mjs`). Earlier painted
> parts-sheet guidance below remains for the record.
